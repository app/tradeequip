...
void Preferences::readData()
{
    ...
    for (uint n=0; n<nodes.count(); ++n) {
        if (nodes.item((int)n).isElement()) {
            processGroup(nodes.item((int)n).toElement());
        }
    }
    formatstate_ = true;
}
...
void Preferences::processGroup(QDomElement group)
{
    ...
    for (uint n=0; n<options.count(); ++n) {
        if (options.item((int)n).isElement()) {
            elem = options.item((int)n).toElement();
