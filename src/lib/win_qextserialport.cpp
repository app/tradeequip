...
\brief Windows serial port class
...
bool Win_QextSerialPort::open(int) {
    ...
    if (!portOpen) {
        ...
        if (Win_Handle!=INVALID_HANDLE_VALUE) {
			...
			confSize=sizeof(Win_CommConfig);
...
Offset Win_QextSerialPort::size() const {
    ...
    availBytes = (int)Win_ComStat.cbInQue;
...
int Win_QextSerialPort::getch() {
    ...
    if (portOpen) {
        ...
        DWORD Win_ErrorMask=0;
        ClearCommError(Win_Handle, &Win_ErrorMask, &Win_ComStat);
        if (true) //(Win_ComStat.cbInQue) { // This statement was bypassing normal timeout's logic, so I removed it.
        {
